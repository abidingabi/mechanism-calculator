...
Therefore, a linear force acting on something with a mass $m$ at a radius $r$ has a moment of inertia of $mr^2$. Assuming that the load torque is also at the same radius, it can be represented as $F_{load}r$. Substituting this into the previously derived linear velocity/position functions:

#+begin_src python
mass = sym.symbols('m')

format_equations(
    linear_velocity_function.subs(moment_of_inertia, mass*radius**2),
    linear_position_function.subs(moment_of_inertia, mass*radius**2)
)
#+end_src
...
:results:
\begin{equation}\begin{aligned}
v{\left(t \right)} &= r \left(- \frac{\omega_{free} \tau_{load}}{\tau_{stall}} + \frac{\omega_{free} \tau_{load} e^{- \frac{t \tau_{stall}}{m \omega_{free} r^{2}}}}{\tau_{stall}} + \omega_{free} - \omega_{free} e^{- \frac{t \tau_{stall}}{m \omega_{free} r^{2}}} + \omega_{i} e^{- \frac{t \tau_{stall}}{m \omega_{free} r^{2}}}\right)\\
x{\left(t \right)} &= r \left(\frac{m \omega_{free}^{2} r^{2} \tau_{load}}{\tau_{stall}^{2}} - \frac{m \omega_{free}^{2} r^{2} \tau_{load} e^{- \frac{t \tau_{stall}}{m \omega_{free} r^{2}}}}{\tau_{stall}^{2}} - \frac{m \omega_{free}^{2} r^{2}}{\tau_{stall}} + \frac{m \omega_{free}^{2} r^{2} e^{- \frac{t \tau_{stall}}{m \omega_{free} r^{2}}}}{\tau_{stall}} + \frac{m \omega_{free} \omega_{i} r^{2}}{\tau_{stall}} - \frac{m \omega_{free} \omega_{i} r^{2} e^{- \frac{t \tau_{stall}}{m \omega_{free} r^{2}}}}{\tau_{stall}} - \frac{\omega_{free} t \tau_{load}}{\tau_{stall}} + \omega_{free} t + \theta_{i}\right)
\end{aligned}\end{equation}
:end:
